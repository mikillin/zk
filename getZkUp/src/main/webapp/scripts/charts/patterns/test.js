...
heatMapElement.style.width = overallLength > 500 ? "500px" : overallLength + "px";
...
for (var i = 0; i &lt; src.length; i++) {
    ...
    for (var j = 0; j &lt; src[i].data.length; j++) {
        ...
        tmpDiv.addEventListener("click", function (event) {
            ...
            let bottomHint = "Aktivitaet:" + event.target.dataset.type +
                ". Datum:" + event.target.dataset.date +
                ". Auswertung:" + event.target.dataset.value +
                ". Ziel:" + optimalValue + ".";
            document.getElementById("tooltip").innerHTML = bottomHint;
        }, false);
